...
class FeatureExtractors(torch.nn.Module):
    def __init__(self, device, 
        ...
        local_ckpt_path = os.path.join(os.path.dirname(__file__), "..", "checkpoints", "feature_extractors", "dino_vitbase8_pretrain.pth")
        try:
            self.rgb_backbone = timm.create_model(model_name = rgb_backbone_name, pretrained = False, **kwargs)
            ckpt = torch.load(local_ckpt_path, map_location=self.device)
            if 'state_dict' in ckpt:
                state_dict = ckpt['state_dict']
            else:
                state_dict = ckpt
            self.rgb_backbone.load_state_dict(state_dict, strict=False)
            print(f"Loaded RGB backbone from local checkpoint: {local_ckpt_path}")
        except Exception as e:
            print(f"Warning: Could not load local checkpoint: {e}")
            # Fallback to pretrained=False (no network required)
            self.rgb_backbone = timm.create_model(model_name = rgb_backbone_name, pretrained = False, **kwargs)
            print("RGB backbone initialized with random weights")
        ...
        pt_ckpt = os.path.join(os.path.dirname(__file__), "..", "checkpoints", "feature_extractors", "pointmae_pretrain.pth")
        self.xyz_backbone.load_model_from_ckpt(pt_ckpt)
        # ! Use only the first k blocks.
...
def fps(data, number):
    ...
    # fps_idx is (B, npoint), data.transpose(1,2) is (B, 3, N)
