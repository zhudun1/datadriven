...
class ResourceLeaseManager:
    ...
    def reserve_node_resources(self, task_id: str, data_id: int, node_demands: dict, resource_req: dict) -> tuple[bool, dict]:
        """预留节点资源（使用乐观锁减少锁冲突）"""
        if not node_demands:
        ...
        try:
            with conn.cursor() as c:
                ...
                for resource_id in resource_ids:
                    ...
                    node_id = int(resource_id.split("-")[-1]) - 1  # edge-1 -> 0
                    demand = node_demands.get(node_id, {"vcpu": 0.0, "memory_gb": 0.0})
                ...
                # 插入分配记录
                # 注意：link_path在编排完成后通过UPDATE更新
                c.execute(f"""
                    INSERT INTO {SERVICE_ALLOCATION_TABLE}
                    (task_id, data_id, status, node_allocation, requested_resource, link_path)
                    VALUES (%s, %s, %s, %s, %s, %s)
                """, (
                    task_id,
                    data_id,
                    ...
                    json.dumps({}),  # 先写入空的link_path，编排后更新
                ))
                allocation_id = int(c.lastrowid)

            conn.commit()
            return True, {"allocation_id": allocation_id, "node_allocation": allocation_snapshot}
                ...
                for row in rows:
                    ...
                    for link_idx, link_info in link_path.items():
                        ...
                        if link_id is not None:
                            ...
                            link_row = c.fetchone()
                            if link_row:
                                current_used_bw = _to_float(link_row.get("used_bw", 0.0), 0.0)
                                new_used_bw = max(0, current_used_bw - release_bw)
                                new_allocations = max(0, int(link_row.get("allocations", 0) or 0) - 1)
                                # UPDATE t_physical_link 列
                                c.execute("""
                                    UPDATE t_physical_link
                                    SET used_bw = %s, allocations = %s
                                    WHERE link_id = %s
                                """, (round(new_used_bw, 4), new_allocations, link_id))
    ...
    def get_seconds_until_candidate_release(self, resource_ids: list, max_wait_seconds: float) -> Optional[float]:
        ...
        window = max(1, int(max_wait_seconds))
        cond_parts = []
        params = [window]
        for resource_id in resource_ids:
            cond_parts.append("JSON_EXTRACT(node_allocation, %s) IS NOT NULL")
            params.append(f'$.\"{resource_id}\"')
        cond_sql = " OR ".join(cond_parts) if cond_parts else "1=0"
        ...
        try:
            with conn.cursor() as c:
                c.execute(f"""
                    ...
                    WHERE status='active'
                      AND release_time IS NOT NULL
                      AND release_time <= TIMESTAMPADD(SECOND, %s, NOW())
                      AND ({cond_sql})
                """, tuple(params))
                row = c.fetchone() or {}
        ...
        try:
            ...
            while True:
                reserved, reservation = self.lease_manager.reserve_node_resources(
                    task_id, data_id, node_demands, resource_req
                )
                if reserved:
                    break
                    ...
                    with conn.cursor() as c:
                        ...
                        for link_idx, link_info in link_path.items():
                            ...
                            if link_id is not None:
                                ...
                                row = c.fetchone()
                                if row:
                                    current_used_bw = _to_float(row.get("used_bw", 0.0), 0.0)
                                    new_used_bw = round(current_used_bw + bw_demand, 4)
                                    new_allocations = int(row.get("allocations", 0) or 0) + 1
                                    # UPDATE t_physical_link 列
                                    c.execute("""
                                        UPDATE t_physical_link
                                        SET used_bw = %s, allocations = %s
                                        WHERE link_id = %s AND is_active = 1
                                    """, (new_used_bw, new_allocations, link_id))
            ...
            # 调度资源释放（1分钟后）
            self.lease_manager.schedule_release(allocation_id, self.lease_manager.hold_seconds)
            ...
            if allocation_id:
                # 改为异步释放，不在主请求中同步执行
                self.lease_manager.schedule_release(allocation_id, hold_seconds=60)
